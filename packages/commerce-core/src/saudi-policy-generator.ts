...
export class SaudiPolicyGenerator {
  ...
  private generatePrivacy(input: PolicyGeneratorInput, warnings: string[]): GeneratedPolicy {
...
للاستفسارات: ${contactEmail}`;
  ...
  private generateTerms(input: PolicyGeneratorInput, warnings: string[]): GeneratedPolicy {
    const storeName = input.storeName || 'المتجر';
...
يخضع هذا الاتفاق لقوانين المملكة العربية السعودية.`;
  ...
  private generateShipping(input: PolicyGeneratorInput, warnings: string[]): GeneratedPolicy {
    const storeName = input.storeName || 'المتجر';
    const policyWarnings: string[] = [];
...
يجب أن يكون عنوان التوصيل دقيقًا وكمالياً. لا يتحمل المتجر مسؤولية التأخير الناتج عن عنوان غير صحيح.`;
  ...
  private generateReturns(input: PolicyGeneratorInput, errors: string[], warnings: string[]): GeneratedPolicy {
    const storeName = input.storeName || 'المتجر';
...
يتحمل العميل تكلفة الشحن في حالة الإرجاع除非 كان السبب خطأ في المنتج.`;
  ...
  private generateAbout(input: PolicyGeneratorInput, warnings: string[]): GeneratedPolicy {
    const storeName = input.storeName || 'المتجر';
    const legalName = input.legalName || storeName;
