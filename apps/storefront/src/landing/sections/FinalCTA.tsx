...
function HighlightNumbers({ text, size = 'inherit' }: { text: string; size?: string }) {
  ...
  return (
    <>
      {parts.map((part, i) =>
        /^\d[\d,]*\+?$/.test(part)
          ? <span key={i} className={`inline-block bg-gradient-to-br from-warning to-warning bg-clip-text text-transparent drop-shadow-[0_0_12px_rgba(251,191,36,0.4)] ${size}`}>{part}</span>
    ...
    <section className="relative py-16 sm:py-24">
      <StoreContainer>
        <div className="aurora-cta relative overflow-hidden rounded-[2.5rem] px-6 py-16 text-center shadow-2xl sm:px-12 sm:py-20">
          ...
          <div aria-hidden="true" className="absolute -end-32 -top-32 h-96 w-96 rounded-pill bg-primary-400/40 blur-3xl animate-pulse motion-reduce:animate-none" style={{ animationDuration: '6s' }} />
          <div aria-hidden="true" className="absolute -bottom-32 -start-32 h-96 w-96 rounded-pill bg-primary-400/30 blur-3xl animate-pulse motion-reduce:animate-none" style={{ animationDuration: '8s' }} />
          <div aria-hidden="true" className="absolute left-1/2 top-1/2 h-64 w-64 -translate-x-1/2 -translate-y-1/2 rounded-pill bg-primary-400/20 blur-3xl animate-pulse motion-reduce:animate-none" style={{ animationDuration: '10s' }} />
          {/* Floating particles */}
          {particles.map((p) => (
            <div
              key={p.id}
          ...
          <div className="relative mx-auto max-w-4xl">
            <span className="inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-eyebrow text-white/90 backdrop-blur-sm">
              <span className="h-1.5 w-1.5 rounded-pill bg-success shadow-[0_0_8px_rgba(110,231,183,0.8)] animate-pulse motion-reduce:animate-none" />
            ...
            <h2 className="mt-7 leading-[1.15] tracking-[-0.03em]">
              <div className="flex flex-wrap items-baseline justify-center gap-x-3 gap-y-0">
                ...
                <span className="text-[72px] font-extrabold bg-gradient-to-br from-warning to-warning bg-clip-text text-transparent drop-shadow-[0_0_20px_rgba(251,191,36,0.5)] sm:text-[96px] lg:text-[120px] xl:text-[140px]">
            ...
            <div className="mt-10 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:items-center sm:gap-4">
              <Link
                ...
                className="group inline-flex h-14 min-h-[56px] w-full items-center justify-center gap-2 rounded-full bg-white px-8 text-[17px] font-semibold text-primary-700 shadow-xl transition-all motion-reduce:transition-none duration-300 hover:scale-[1.04] hover:shadow-2xl sm:w-auto"
              >
            ...
            <ul className="mt-10 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-sm font-medium text-white/80">
              ...
              <li className="flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 backdrop-blur-sm transition-all motion-reduce:transition-none hover:bg-white/20 hover:text-white">
                <Rocket className="h-4 w-4 text-primary-300" />
