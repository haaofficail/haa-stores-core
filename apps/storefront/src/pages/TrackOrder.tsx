...
export default function TrackOrder() {
  ...
  const handleTrack = () => {
    ...
    sessionStorage.setItem(`track_phone_${orderNumber.trim()}`, phone.trim());
    navigate(`/s/${slug}/track/${orderNumber.trim()}`);
  };
    ...
    <div id="main-content" className="animate-fade-in overflow-x-hidden">
      <section className="bg-gradient-to-bl from-primary-50 via-white to-primary-50/30 py-12 sm:py-16">
        <StoreContainer className="text-center">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-text-primary mb-3">{t('track.title')}</h1>
          <p className="text-lg text-text-secondary max-w-md mx-auto">{t('track.description')}</p>
        </StoreContainer>
      </section>
      ...
      <StoreContainer className="py-8 sm:py-10">
        <div className="max-w-md mx-auto">
          ...
          <div className="text-center mt-6">
            <Link to={`/s/${slug}`} className="inline-flex items-center gap-2 text-sm text-primary-600 hover:text-primary-700 font-medium">
