...
export default function AccountantInbox() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      if (q && !r.merchantName.toLowerCase().includes(q) && !r.reference.toLowerCase().includes(q)) return false;
      if (statusFilter && r.status !== statusFilter) return false;
      if (periodFilter && (r.period ?? '') !== periodFilter) return false;
      return true;
    });
  }, [rows, merchantQuery, statusFilter, periodFilter]);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={() => downloadRowsAsCsv(filtered as unknown as Record<string, unknown>[], `settlement-${segment}.csv`)}
