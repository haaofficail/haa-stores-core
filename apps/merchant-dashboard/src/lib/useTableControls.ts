/* eslint-disable @typescript-eslint/no-explicit-any -- list rows carry legacy `any` typing on API responses; this table-controls hook is intentionally generic over them. */
...
interface UseTableControlsOptions<T> {
  ...
  pageSize?: number;
  /** When set, query + sort + pageSize survive reloads under `haa-merchant-table:<storageKey>`. */
  storageKey?: string;
  /** Custom accessor for sortable cell values (e.g. nested or computed fields). */
  getValue?: (row: T, key: string) => unknown;
}

const STORAGE_PREFIX = 'haa-merchant-table:';
 ...
 * with optional localStorage persistence. For list endpoints that return full
 * arrays, all processing happens in the browser.
 */
