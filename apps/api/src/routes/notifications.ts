...
notificationsRouter.get('/preferences', requirePermission('settings:read'), async (c) => {
  const storeId = Number(c.req.param('storeId'));
  const prefs = await new NotificationService().getPreferences(storeId);
  return c.json({ success: true, data: prefs });
});

notificationsRouter.put('/preferences', requirePermission('settings:update'), zValidator('json', z.object({
...
notificationsRouter.get('/logs', requirePermission('settings:read'), async (c) => {
  const storeId = Number(c.req.param('storeId'));
...
notificationsRouter.get('/templates', requirePermission('settings:read'), async (c) => {
