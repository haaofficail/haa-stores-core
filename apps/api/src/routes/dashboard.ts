...
dashboardRouter.get('/summary', requirePermission('dashboard:view'), async (c) => {
  ...
  const products = await db.select({ total: count() }).from(s.products)
    .where(eq(s.products.storeId, storeId));
  const activeProducts = await db.select({ total: count() }).from(s.products)
    .where(and(eq(s.products.storeId, storeId), eq(s.products.status, 'active')));
  const lowStockProducts = await db.select({ total: count() }).from(s.products)
    .where(and(eq(s.products.storeId, storeId), eq(s.products.trackInventory, true), sql`${s.products.stockQuantity} <= 5`));

  const orders = await db.select({ total: count() }).from(s.orders)
    .where(eq(s.orders.storeId, storeId));
  const newOrders = await db.select({ total: count() }).from(s.orders)
    .where(and(eq(s.orders.storeId, storeId), eq(s.orders.status, 'pending_payment')));

  const paidOrdersTotal = await db.select({ total: sql<string>`COALESCE(SUM(${s.orders.total}), 0)` }).from(s.orders)
    .where(and(eq(s.orders.storeId, storeId), eq(s.orders.paymentStatus, 'paid')));

  const wallet = await db.select().from(s.walletAccounts)
    .where(eq(s.walletAccounts.storeId, storeId)).limit(1);
  const account = wallet[0];

  const shippingMethods = await db.select({ total: count() }).from(s.shippingMethods)
  ...
  const notExternal = not(inArray(s.orders.source, ['salla', 'zid', 'noon', 'amazon']));

  const newOrdersCount = await db.select({ total: count() }).from(s.orders)
    .where(and(storeFilter, or(eq(s.orders.status, 'pending_payment'), eq(s.orders.status, 'confirmed'))));

  const readyToShipCount = await db.select({ total: count() }).from(s.orders)
    .where(and(storeFilter, eq(s.orders.status, 'ready_to_ship'), not(eq(s.orders.fulfillmentType, 'local_pickup')), notExternal));

  const readyForPickupCount = await db.select({ total: count() }).from(s.orders)
    .where(and(storeFilter, eq(s.orders.status, 'ready_for_pickup'), eq(s.orders.fulfillmentType, 'local_pickup'), notExternal));

  const codCollectionCount = await db.select({ total: count() }).from(s.orders)
    .where(and(storeFilter, eq(s.orders.paymentMethod, 'cash_on_delivery'), eq(s.orders.paymentStatus, 'pending'),
      or(eq(s.orders.status, 'delivered'), eq(s.orders.status, 'picked_up'))));

  const outOfStockCount = await db.select({ total: count() }).from(s.products)
    .where(and(eq(s.products.storeId, storeId), eq(s.products.trackInventory, true), sql`${s.products.stockQuantity} <= 0`));

  const lowStockOnlyCount = await db.select({ total: count() }).from(s.products)
    .where(and(eq(s.products.storeId, storeId), eq(s.products.trackInventory, true), sql`${s.products.stockQuantity} > 0`, sql`${s.products.stockQuantity} <= 5`));
  ...
  const actionableOrders = await db.select({
    id: s.orders.id,
  ...
  const [activePaymentProviders] = await db.select({ total: count() }).from(s.merchantPaymentProviderSettings)
    .where(and(eq(s.merchantPaymentProviderSettings.storeId, storeId), eq(s.merchantPaymentProviderSettings.enabled, true)));

  const [activePickupLocations] = await db.select({ total: count() }).from(s.pickupLocations)
    .where(and(eq(s.pickupLocations.storeId, storeId), eq(s.pickupLocations.isActive, true)));

  const [activeShippingMethodsCount] = await db.select({ total: count() }).from(s.shippingMethods)
    .where(and(eq(s.shippingMethods.storeId, storeId), eq(s.shippingMethods.isActive, true)));

  const [draftProductsCnt] = await db.select({ total: count() }).from(s.products)
    .where(and(eq(s.products.storeId, storeId), eq(s.products.status, 'draft')));

  const productsWithoutImagesRow = await db.select({ total: count() }).from(s.products)
    .where(and(eq(s.products.storeId, storeId), eq(s.products.status, 'active'),
  ...
  const [kycProfile] = await db.select({ status: s.kycProfiles.status }).from(s.kycProfiles)
    .where(eq(s.kycProfiles.storeId, storeId)).limit(1);

  const policies = await db.select({ type: s.storePolicies.type, isPublished: s.storePolicies.isPublished })
    .from(s.storePolicies).where(and(eq(s.storePolicies.storeId, storeId), eq(s.storePolicies.isPublished, true)));
