...
- [ ] Admin Support Gateway stays platform-admin owned: `/support-gateway` is visible only with `support.gateway.read`, `/admin/support-gateway/tickets` has the matching server guard, the page explains ticket vs landing-inbox scope, and no support `accessToken` is selected or rendered.
