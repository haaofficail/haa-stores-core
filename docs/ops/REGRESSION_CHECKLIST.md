...
- [ ] Admin Store Payment Settings save contract stays aligned: page sends canonical `enabled`/`status`/`supportedPaymentMethod`, API accepts `active`/`suspended`/`not_configured`, inserts default `supportedPaymentMethod: 'card'`, and saved-provider cache patching does not broad-refetch sibling unsaved rows
