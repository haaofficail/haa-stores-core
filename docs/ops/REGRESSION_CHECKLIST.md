...
- [ ] Admin tenant operating dossier remains the primary tenant-level decision surface: `/tenants` links to `/tenants/:tenantId`, the dossier aggregates stores, Merchant Verification readiness, blockers, masked bank state, payment/payout/settlement context, and audit history, while avoiding full IBAN display and payout/transfer/settlement mutations from the overview.
