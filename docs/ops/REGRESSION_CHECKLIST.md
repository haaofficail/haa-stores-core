...
- [ ] Staging deploy runs on the dedicated self-hosted runner labels `[self-hosted, linux, x64, haa-staging]` and does not reintroduce staging `STAGING_SSH_KEY`, `STAGING_HOST`, `STAGING_USER`, `STAGING_SSH_PORT`, `ssh`, or `scp` dependencies; production SSH is a separate path
...
- [ ] Admin login base credential queries must avoid optional migration columns such as `admin_role`; fetch those separately behind schema-readiness fallbacks so local/staging schemas do not lock out admins before owner-applied migrations.
- [ ] Admin users/stores/tenants/settings list routes must use explicit migration-stable column selections; user lists must not select password hashes, admin TOTP secrets, or other unnecessary sensitive fields.
- [ ] Admin store create/edit must stay aligned with the canonical store `slug` contract, use merchant selection instead of raw Tenant ID, and collect required store email before calling the API.
- [ ] Admin sidebar user-management navigation must use `/users`; `/admin-users` may exist only as a redirect alias because browser/client filters can block it before React renders.
...
- [ ] Admin financial exports must also record audit entries, use explicit safe columns, and avoid raw payment metadata/provider references/idempotency keys; settlement readiness UI values must match the API validator before save controls are shown
...
- [ ] Admin SaaS operations UX stays decision-safe: `not_started` / `incomplete` Merchant Verification records must not become `high` risk without actual risk signals; unconfigured payment providers must not send `enabled=true`; Settlement Readiness must show decision, withdrawal permission, blockers, owner, and next action; key empty states must explain meaning and next action.
- [ ] Admin Support Gateway stays platform-admin owned and triage-safe: `/support-gateway` is visible only with `support.gateway.read`, `/admin/support-gateway/tickets` has the matching server guard, the page explains ticket vs landing-inbox scope, each row shows owner/next action, filters can be reset locally, no support mutation actions are introduced, and no support `accessToken` is selected or rendered.
- [ ] Admin tenant operating dossier remains the primary tenant-level decision surface: `/tenants` links to `/tenants/:tenantId`, the dossier aggregates stores, Merchant Verification context, masked bank state, payout/extract context, and tenant-scoped audit history, while avoiding full IBAN display, payout/transfer/settlement mutations, platform-wide payment totals, store-unscoped settlement counts, and false publish-readiness blockers from incomplete source data.
- [ ] Merchant compliance and admin verification stay aligned: dashboard sends canonical `commercialRegistrationNumber`/`nationalIdOrIqama`/`freelanceDocumentNumber`, API accepts legacy aliases only as compatibility, profile responses do not expose full national ID, bank readiness uses `ibanLast4 + status`, admin `/compliance` remains Merchant Verification / "توثيق المتاجر" rather than Platform Launch Gates, and the admin page never displays full IBAN values.
- [ ] Admin Merchant Verification remains actionable, not diagnostic-only: readiness checklist items include admin action destinations and merchant instructions, approval is blocked while readiness blockers exist, request-changes persists `needs_more_info` with a reason, bank review uses masked summaries only, filtered empty results do not show stale selected-store details, and PCI/Pentest/ASV/DR never appear as merchant requirements.
- [ ] Admin Merchant Verification list/detail separation stays intact: `/compliance` is an index with merchant-file links, `/compliance/:recordId` owns the selected merchant dossier, and that file includes approval stages, profile, phone/email, registry/freelance document, bank, documents, payment/shipping/policies, publish readiness, sales/payments, payouts/extracts, settlement batches, review decisions, audit history, and internal notes without full IBAN exposure.
