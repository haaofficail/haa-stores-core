...
- **Actual:** The PR introduced several similarly structured compliance reports and runbooks, while the inherited admin page nested response normalization inside a hook callback. The frontend hook used POSIX `[` in a bash script even though the rest of the hook uses `[[`.
- **Root Cause:** Sonar's new-code quality gate was treating governance docs as CPD input, and the inherited admin code kept data normalization inside nested callbacks instead of reusable helpers.
- **Fix:** Added `sonar-project.properties` with `docs/**` / Markdown CPD exclusions, extracted provider-settings normalization helpers outside `StorePaymentSettings`, and changed the hook file-existence test to `[[ -f "$INDEX_CSS" ]]`.
- **Verification:** `pnpm --filter @haa/admin-dashboard typecheck` passed; `pnpm --filter @haa/admin-dashboard build` passed; focused settlement/geidea tests passed 3 files / 24 tests; `bash -n scripts/hooks/pre-edit-frontend.sh` passed; `pnpm check:skills` passed 43/43; `git diff --check` passed.
- **Prevention:** Keep generated/templated docs out of Sonar CPD scope, keep admin API response normalization in top-level helpers, and use bash `[[ ]]` conditionals in repo hook scripts.
- **Status:** Fixed locally in TASK-0093; awaiting refreshed SonarCloud result after second push.
