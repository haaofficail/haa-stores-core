...
- **Fix:** Added `sonar-project.properties` with `docs/**` / Markdown CPD exclusions, extracted provider-settings normalization helpers outside `StorePaymentSettings`, changed the hook file-existence test to `[[ -f "$INDEX_CSS" ]]`, and narrowed `escapeHtmlText` to string/number/boolean/null/undefined values.
