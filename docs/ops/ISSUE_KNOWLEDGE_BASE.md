...
- **Related Tasks:** TASK-0145, TASK-0146
...
- **Fix:** TASK-0145 first improved the workflow diagnostics: it printed the GitHub runner public IPv4, supported optional `STAGING_SSH_PORT`, validated pre-baked `STAGING_KNOWN_HOSTS`, and produced clearer firewall/provider/alternate-port errors. TASK-0146 then prepared the durable cutover: `Deploy to Staging` now targets `[self-hosted, linux, x64, haa-staging]`, has a staging-specific concurrency group, and runs staging GHCR login/config sync/deploy/diagnostics/rollback locally on the VPS runner instead of using staging SSH key/host/user/port, SSH warmup, `scp`, or `ssh`. It also adds `scripts/server/install-github-runner-staging.sh` so the runner can be registered from the VPS console with the registration token passed only through `GITHUB_RUNNER_TOKEN`.
- **Verification:** Local verification for TASK-0145 passed. Local verification for TASK-0146 so far: workflow YAML parsed and focused deploy workflow tests passed 3 files / 47 tests. GitHub API currently reports zero self-hosted runners for this repository, and local SSH to `deploy@72.61.108.208` fails with `Permission denied (publickey,password)`, so the `haa-staging` runner still must be installed/registered on the VPS before merging the workflow cutover.
- **Prevention:** Staging deploy should run on the self-hosted VPS runner labels `self-hosted`, `linux`, `x64`, `haa-staging` and must not depend on GitHub-hosted inbound SSH. If `Deploy to Staging` queues indefinitely, first check repository runner registration/online status and the `haa-staging` label before changing application code. Production SSH behavior is separate and remains unchanged.
- **Status:** TASK-0146 workflow cutover prepared locally. Merge is blocked until the `haa-staging` self-hosted runner is online; otherwise the staging deploy job will queue waiting for a matching runner.
...
- **Fix:** Added `support.gateway.read` to the admin permission model and catalog, mounted `/admin/support-gateway/tickets` with admin auth and permission guard, moved the read query into `apps/api/src/services/admin-support-gateway.ts`, added an admin API-client helper, added `/support-gateway` to the admin sidebar/router, and built a read-only Support Gateway page with safe summaries, filters, tenant links, and no `accessToken` exposure.
