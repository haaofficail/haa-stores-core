...
- **Status:** VPS runner registered and online; workflow cutover ready for staging deploy verification
- **Created:** 2026-07-02
- **Updated:** 2026-07-02
...
- **Scope:** `.github/workflows/deploy.yml` staging job runner selection, staging job-level concurrency, removal of staging SSH setup/warmup/scp/ssh commands, local-on-runner GHCR login/config sync/deploy/diagnostics/rollback, a server-side runner bootstrap script, focused deploy workflow regression tests, and ops documentation.
...
- **Acceptance Criteria:**
  ...
  - [x] A safe VPS bootstrap script exists to register/install the staging runner without committing or printing the GitHub registration token.
  - [x] A GitHub self-hosted runner with label `haa-staging` is registered on VPS `72.61.108.208`.
...
- **Files Changed:** `.github/workflows/deploy.yml`, `scripts/server/install-github-runner-staging.sh`, `tests/deploy-no-ssh-keyscan.test.ts`, `tests/deploy-hardening.test.ts`, ops documentation, and `docs/ops/SKILL_COMPLIANCE_REPORT_TASK_0146.md`.
- **Test Results:** In the isolated worktree, workflow YAML parsed successfully and focused deploy workflow tests passed 3 files / 47 tests. After owner console execution of `scripts/server/install-github-runner-staging.sh`, GitHub API reports one online self-hosted runner: `haa-staging-vps` with labels `self-hosted`, `Linux`, `X64`, and `haa-staging`.
...
- **Verdict:** Workflow fix is ready for staging deploy verification. The `haa-staging` runner is online; next step is merging PR #355 to `main`, watching `Deploy to Staging`, and verifying smoke. No app-code change, migration, production action, or secret print occurred.
...
- **Scope:** Staging deploy GitHub Actions SSH diagnostics, optional `STAGING_SSH_PORT` configuration defaulting to `22` with legacy `STAGING_PORT` fallback, use of the configured port across staging SSH/SCP calls, source guard update, and ops documentation for the root cause and next operating step.
...
- **Acceptance Criteria:**
  ...
  - [x] Pre-baked `STAGING_KNOWN_HOSTS` is validated against the configured port before SSH warmup.
...
- **Test Plan:** `ruby -e "require 'yaml'; YAML.load_file('.github/workflows/deploy.yml'); puts 'yaml ok'"`; `pnpm vitest run tests/deploy-no-ssh-keyscan.test.ts`; `pnpm check:skills`; `git diff --check`; `CI=true pnpm preflight`; PR checks after push. Plain `pnpm preflight` is expected to fail in this isolated worktree because the root guard is hard-coded to `/Users/thwany/Desktop/haa-stores-core`.
- **Files Changed:** `.github/workflows/deploy.yml`, `tests/deploy-no-ssh-keyscan.test.ts`, ops documentation, and `docs/ops/SKILL_COMPLIANCE_REPORT_TASK_0145.md`.
- **Test Results:** Local verification passed: workflow YAML parsed (`yaml ok`), focused deploy guard test passed, `git diff --check` was clean, `pnpm check:skills` passed 43/43, and `CI=true pnpm preflight` passed. Plain `pnpm preflight` correctly failed in the isolated worktree on the root-guard path check only. First PR check surfaced the old source guard expecting `scp -o BatchMode=yes`; it was updated to guard `scp -P "$DEPLOY_PORT" -o BatchMode=yes` and staging configurable-port behavior. PR review then surfaced that custom-port `STAGING_KNOWN_HOSTS` must use `[host]:port`; workflow now validates that with `ssh-keygen -F` before warmup.
...
- **Type:** UX/UI Polish / Testing / Documentation
- **Priority:** P1 High
- **Status:** PR #351 updated with review-thread fixes and Support Gateway visibility fix; local verification passed; remote checks pending after rebase
- **Created:** 2026-07-01
- **Updated:** 2026-07-01
...
- **Scope:** Admin sidebar IA, dashboard command center, Merchant Verification risk vocabulary, Settlement Readiness decision UX, Store Payment Settings readiness vocabulary, smart empty states on decision-heavy admin pages, the platform-admin Support Gateway visibility fix, focused source/behavior tests, and ops documentation.
- **Out of Scope:** Deploy, staging/production changes, migrations, production config, secrets, live provider calls, API changes outside the read-only Support Gateway contract, owner-level pricing decisions, full table/drawer redesign across every admin page, and finance destructive-action changes already tracked under TASK-0140.
...
- **Files Changed:** `apps/admin-dashboard/src/App.tsx`, `apps/admin-dashboard/src/components/ui/AdminEmptyState.tsx`, `apps/admin-dashboard/src/lib/merchantVerification.ts`, selected `apps/admin-dashboard/src/pages/**`, `apps/admin-dashboard/src/pages/SupportGateway.tsx`, `apps/admin-dashboard/src/lib/api.ts`, `apps/api/src/routes/admin/support-gateway.ts`, `apps/api/src/services/admin-support-gateway.ts`, `packages/shared/src/permissions.ts`, `packages/shared/src/types/orders.ts`, `tests/admin-dashboard-saas-ux.test.ts`, `tests/admin-support-gateway.test.ts`, existing admin tests, `docs/ops/ADMIN_DASHBOARD_SAAS_UX_AUDIT_2026-07-01.md`, `docs/ops/SKILL_COMPLIANCE_REPORT_TASK_0142.md`, and ops docs.
- **Test Results:** Verification passed: original TASK-0142 UX checks, review-thread fix checks, and Support Gateway follow-up checks. Latest local verification included `pnpm vitest run tests/admin-support-gateway.test.ts tests/admin-permission-reflection.test.ts tests/admin-dashboard-saas-ux.test.ts tests/service-layer-enforcement.test.ts` passing 4 files / 25 tests; `pnpm --filter @haa/api typecheck`; `pnpm --filter @haa/admin-dashboard typecheck`; `pnpm --filter @haa/admin-dashboard build`; `pnpm check:skills` passing 43/43; clean `git diff --check`; and `CI=true pnpm preflight`. Browser verification on `http://127.0.0.1:5176/support-gateway` confirmed the heading, sidebar link, API request, `/landing-inbox` bridge, smart empty state, and no access-token text. Previous PR #351 project-owned checks were green before the latest rebase; remote checks are pending again for the rebased support-gateway update. External/account-tooling blockers remain: Snyk private-test limit and TestSprite `No tests detected`.
...
- **Support Gateway Follow-up:** Root cause confirmed: the admin support gateway was not hidden by styling; it was absent from the platform-admin contract. Existing `support:*` permissions/routes are merchant/store-scoped, and admin only had `/landing-inbox` for lead inbox data. TASK-0142 now adds `support.gateway.read`, `/support-gateway`, `/admin/support-gateway/tickets`, service-layer admin support-ticket read model, admin API-client helpers, a safe Arabic read-only page, and tests ensuring no support `accessToken` is selected or rendered. The CI Test failure after the first push was caused by the new route importing `drizzle-orm` directly; the query was moved into `apps/api/src/services/admin-support-gateway.ts` so the service-layer guard stays at its existing budget.
- **Verdict:** PR #351 is updated from isolated TASK-0142 worktree `/Users/thwany/Desktop/haa-stores-core-task-0142` and rebased over latest `origin/main`. The branch contains admin-dashboard UX, support gateway, tests, and TASK-0142 docs only; no TASK-0143 API cleanup files are included. No manual deploy, migration, DB mutation, production action, secret handling, or live payment/shipping/provider call occurred.
- **Related Issues:** ISSUE-0078, ISSUE-0080.
...
- **Priority:** P1 High
- **Status:** Merged via PR #346; main CI and staging deploy verified
- **Created:** 2026-07-01
- **Updated:** 2026-07-01
- **Branch:** `fix/main-change-password-tenant-id`
- **PR:** #346
- **Original Request:** "أكمل الآن كـ Main CI Blocker Fix فقط" for `apps/api/src/routes/auth.ts(452,9): 'tenantId' does not exist in type 'ChangePasswordInput'`.
- **Expanded Requirement:** Fix the current `origin/main` TypeScript/preflight blocker in an independent branch from `origin/main`, without mixing TASK-0134..0140 stack changes, deploy, migration, production config, secrets, runtime files, or test suppression.
- **Scope:** Merchant change-password route type wiring and focused account-security regression coverage.
- **Out of Scope:** TASK-0134..0140 stack changes, production deploy, `db:migrate`, secrets, production config, live provider calls, and broad auth refactors.
- **Skills Used:** `agent-permission-boundary`, `environment-safety-gate`, `regression-safety-gate`, `acceptance-criteria-gate`, `test-strategy-gate`, `branch-pr-hygiene-gate`, `verification-before-completion`, `evidence-led-reporting`, and `hono-typescript`.
- **Acceptance Criteria:**
  - [x] Branch is created cleanly from `origin/main`.
  - [x] Fix is independent and does not include TASK-0134..0140 stack changes.
  - [x] `tenantId`/`storeId` remain server-side JWT context for audit logging, not request payload schema fields.
  - [x] `pnpm typecheck` passes.
  - [x] `pnpm preflight` passes.
  - [x] No runtime files, secrets, manual migration, production deploy, or production action.
- **Test Plan:** `pnpm ops:monitor`; `pnpm --filter @haa/api typecheck`; `pnpm vitest run tests/merchant-account-security.test.ts tests/route-migration-1-auth.test.ts tests/auth-regression.test.ts tests/account-page-contract.test.ts`; `pnpm typecheck`; `pnpm preflight`; `pnpm --filter @haa/api build`; `pnpm check:skills`; `git diff --check`; post-merge CI/deploy watch.
...
- **Test Results:** Local verification passed before PR. PR #346 merged into `main` at `f5af0cbc86681f5d1edbb703e03638b02a7180e5`; GitHub CI run `28485302991` passed; Deploy run `28485302982` passed staging and smoke 5/5; `Deploy to Production` was skipped; local `main` `pnpm typecheck` and `pnpm preflight` passed after merge.
- **Root Cause:** PR #345 added server-side tenant/store context to `AuthFlowService.changePassword()` and the `/auth/change-password` call. The source interface already accepted `tenantId` and `storeId`, but local/API type resolution used the package export declaration from ignored `packages/commerce-core/dist`, where `ChangePasswordInput` was stale. The inline object literal in `auth.ts` triggered TypeScript excess-property checking against that stale declaration.
- **Verdict:** Merged and staging-verified. Stack branches remain unpushed pending separate refresh and report.
- **Related Issues:** ISSUE-0075.
...
- **Out of Scope:** New migrations, deploy, production config, secrets, live payment/shipping/provider calls, payout approval, transfer verification, settlement mutation, full IBAN display, and the TASK-0140 financial destructive-action hardening scope.
- **Skills Used:** `acceptance-criteria-gate`, `design-ux-excellence-gate`, `regression-safety-gate`, `test-strategy-gate`, `single-source-of-truth-gate`, `verification-before-completion`, `documentation-handoff-gate`, `evidence-led-reporting`, `environment-safety-gate`, `branch-pr-hygiene-gate`, and `agent-permission-boundary`.
- **Acceptance Criteria:**
...
### TASK-0138: Admin dashboard deep QA route/action hardening

- **Type:** UX/UI Polish / Backend API / Testing / Documentation
- **Priority:** P1 High
...
- **PR:** Not opened
- **Original Request:** User supplied a deep admin-dashboard QA brief: inspect every button/link/filter/action as product operations, prioritize Merchant Verification, prevent wrong approvals/sensitive data exposure, fix P0/P1 issues, test manually, and report readiness.
- **Expanded Requirement:** Audit admin navigation and Merchant Verification as an operational admin workflow, verify each high-risk interaction against product goals, fix confirmed P1 blockers in the current branch, and document remaining risks without deploying or running migrations.
- **Scope:** Admin sidebar route smoke, Users route, Stores create/edit workflow, admin list/settings/user API selection stability, Merchant Verification no-results and merchant-file safety checks, source regression tests, browser verification, and ops documentation.
- **Out of Scope:** DB migrations, production/staging deploy, production config changes, secrets, live payment/shipping/provider calls, adding new permission migrations, submitting live approve/reject/bank-review mutations without suitable local reviewable seed data, and broad finance destructive-action rewrites.
- **Skills Used:** `acceptance-criteria-gate`, `design-ux-excellence-gate`, `premium-product-quality-council`, `priority-triage-gate`, `regression-safety-gate`, `implementation-quality-gate`, `test-strategy-gate`, `single-source-of-truth-gate`, `verification-before-completion`, `documentation-handoff-gate`, `evidence-led-reporting`, `environment-safety-gate`, `branch-pr-hygiene-gate`, `agent-permission-boundary`, plus `redesign-existing-projects`, `build-web-apps:frontend-testing-debugging`, `build-web-apps:react-best-practices`, and `browser:control-in-app-browser`.
- **Acceptance Criteria:**
  - [x] Primary admin routes are browser-smoked locally and do not show failed-query errors.
  - [x] Sidebar user route uses `/users`, not browser-blocked `/admin-users`.
  - [x] Stores create/edit uses canonical `slug`, merchant select, required email, and optional phone.
  - [x] Admin users/stores/tenants/settings list routes avoid broad `select *` patterns on migration-optional schemas.
  - [x] Merchant Verification no-result search does not show stale merchant details.
  - [x] Merchant Verification file does not display full IBAN and does not mix PCI/Pentest/ASV/DR into merchant requirements.
  - [x] Unsafe review actions remain disabled for incomplete/non-reviewable local sample merchants.
  - [x] Focused regression tests cover the newly fixed route/action contracts.
- **Test Plan:** `pnpm vitest run tests/admin-action-routing-integrity.test.ts tests/admin-permission-reflection.test.ts tests/accountant-inbox-page.test.ts tests/admin-dangerous-action-reasons.test.ts tests/admin-merchant-verification.test.ts`; `pnpm --filter @haa/api typecheck`; `pnpm --filter @haa/admin-dashboard typecheck`; `pnpm --filter @haa/admin-dashboard build`; in-app browser route/action smoke; `pnpm check:skills`; `git diff --check`; final `pnpm preflight`.
- **Files Changed:** `apps/admin-dashboard/src/App.tsx`, `apps/admin-dashboard/src/lib/api.ts`, `apps/admin-dashboard/src/pages/Stores.tsx`, `apps/api/src/routes/admin/index.ts`, `apps/api/src/routes/admin/tenants-stores.ts`, `apps/api/src/routes/admin/operations.ts`, `tests/admin-action-routing-integrity.test.ts`, `tests/admin-permission-reflection.test.ts`, `tests/accountant-inbox-page.test.ts`, `docs/ops/ADMIN_DASHBOARD_DEEP_QA_TASK_0138.md`, plus ops state/changelog/issue/regression/compliance docs.
- **Test Results:** Focused source suite passed 5 files / 37 tests. `pnpm --filter @haa/api typecheck` passed. `pnpm --filter @haa/admin-dashboard typecheck` passed. `pnpm --filter @haa/admin-dashboard build` passed. Browser verification passed across 21 primary admin routes with no visible failed-query errors or full IBAN patterns; `/stores` add modal shows corrected fields and validation; `/compliance` no-results hides stale details; `/compliance/store-1` keeps unsafe review actions disabled for the incomplete local sample and contains no PCI/ASV/Pentest/DR merchant requirements. `pnpm check:skills` passed 43/43. `git diff --check` was clean. Final `pnpm preflight` passed.
- **Root Cause:** The admin shell and Stores page still had operational drift after the Merchant Verification redesign: one nav path was filter-blocked, store create/edit used a stale `domain` mental model rather than the API `slug` contract, and several admin API list routes selected full schema rows even when pages needed only stable columns.
- **Verdict:** P1 route/action hardening is implemented and locally verified. Not pushed, not opened as PR, not merged, and not deployed.
- **Related Issues:** ISSUE-0074.

---

### TASK-0137: Admin merchant dossier route and approval stages
...
- **Branch:** `codex/merchant-compliance-readiness-fix`
- **PR:** Not opened
- **Original Request:** User clarified that the admin must be able to choose one merchant from the list and open a professional merchant file containing verification, edits, history, sales, payouts/extracts, settlements, and all merchant data. User also emphasized that approval stages such as bank, registry, phone, email, payment, shipping, policies, and publish readiness must be visible and actionable.
- **Expanded Requirement:** Convert admin `/compliance` from a list with inline/global detail behavior into a Merchant Verification index plus a dedicated merchant dossier route. The index should only help reviewers find and open a merchant; `/compliance/:recordId` should become the full operational file for the selected merchant.
- **Scope:** Admin route activation, merchant-file link behavior, merchant dossier tabs, approval-stage board, profile/verification/bank/document details, payment/shipping/publish readiness, sales/payments, payouts/extracts, settlement batches, audit/review history, internal notes, focused regression tests, browser verification, and ops documentation.
- **Out of Scope:** DB migrations, production/staging deploy, production config changes, adding new permission migrations, live payment/shipping/provider calls, full IBAN display, deleting Platform Launch Gates, or mixing PCI/Pentest/ASV/DR into merchant verification.
- **Skills Used:** `acceptance-criteria-gate`, `design-ux-excellence-gate`, `premium-product-quality-council`, `regression-safety-gate`, `implementation-quality-gate`, `test-strategy-gate`, `single-source-of-truth-gate`, `verification-before-completion`, `documentation-handoff-gate`, `evidence-led-reporting`, `environment-safety-gate`, `branch-pr-hygiene-gate`, `agent-permission-boundary`, plus `redesign-existing-projects`, `build-web-apps:react-best-practices`, and `browser:control-in-app-browser`.
- **Acceptance Criteria:**
  - [x] `/compliance` remains a merchant-verification index and no longer renders all selected merchant details under the global table.
  - [x] Store name and review action open a dedicated merchant file route: `/compliance/:recordId`.
  - [x] Sidebar/nav active state treats merchant-file child routes as part of "توثيق المتاجر".
  - [x] Merchant file shows approval stages in front of the admin: store data, phone, email, registry/freelance document, bank, payment, shipping, policies, risk, and publish decision.
  - [x] Merchant file includes clear tabs for approval stages, merchant data, operational activation, sales/payouts/settlements, review/audit history, and internal notes.
  - [x] Bank summary stays masked with `ibanLast4`; no full IBAN is displayed.
  - [x] Admin can reach the relevant review/operational pages from the merchant file.
  - [x] Finance tab scopes visible sales/payments, payouts/extracts, and settlement batches to the selected store when data is available.
  - [x] PCI/Pentest/ASV/DR remain absent from merchant verification.
  - [x] Browser verification confirms list/detail separation and merchant-file tabs.
- **Test Plan:** `pnpm --filter @haa/admin-dashboard typecheck`; focused compliance/platform/RBAC/auth regression suite; in-app browser verification of `http://localhost:5175/compliance` and `http://localhost:5175/compliance/store-1`; `pnpm --filter @haa/admin-dashboard build`; `pnpm check:skills`; `git diff --check`; final `pnpm preflight`.
- **Files Changed:** `apps/admin-dashboard/src/App.tsx`, `apps/admin-dashboard/src/pages/Compliance.tsx`, `tests/admin-merchant-verification.test.ts`, `docs/agent-os/ACTIVE_WORK.md`, `docs/ops/TASK_TRACKER.md`, `docs/ops/CURRENT_STATE.md`, `docs/ops/ISSUE_KNOWLEDGE_BASE.md`, `docs/ops/REGRESSION_CHECKLIST.md`, `docs/ops/CHANGELOG_INTERNAL.md`, `docs/ops/SKILL_COMPLIANCE_REPORT_TASK_0137.md`.
- **Test Results:** `pnpm --filter @haa/admin-dashboard typecheck` passed. `pnpm vitest run tests/admin-merchant-verification.test.ts` passed 1 file / 14 tests after the final UI text changes. Focused regression suite passed 6 files / 56 tests: `tests/admin-merchant-verification.test.ts`, `tests/admin-platform-compliance-gates.test.ts`, `tests/admin-permission-reflection.test.ts`, `tests/merchant-compliance-contract.test.ts`, `tests/g1-g10-engineering-prep.test.ts`, and `tests/admin-accountant-login.test.ts`. `pnpm --filter @haa/admin-dashboard build` passed. `pnpm check:skills` passed 43/43. `git diff --check` was clean. Final `pnpm preflight` passed. Browser verification passed: `/compliance` renders a table-only index with `/compliance/store-*` review links, and `/compliance/store-1` renders approval stages, profile, operations, finance, history, notes, and no full IBAN.
- **Root Cause:** TASK-0136 made merchant verification actionable, but the admin experience still needed a clearer information architecture: the merchant list should be an index, while a selected merchant deserves its own file that collects verification, operations, finance, history, and internal notes in one place.
- **Verdict:** Merchant dossier route is implemented and locally verified. Not pushed, not opened as PR, not merged, and not deployed.
- **Related Issues:** ISSUE-0073.

---

### TASK-0136: Admin merchant verification decision workflow hardening
...
- **Branch:** `codex/merchant-compliance-readiness-fix`
- **PR:** Not opened
- **Original Request:** User asked Codex to act as quality auditor and full executor for the admin merchant verification journey, with the product bar: "هل التاجر بيرضى على هذا فعلا و يقول هذي منصة ممتازة جدا و ينصح فيها؟"
- **Expanded Requirement:** Upgrade admin `/compliance` from a mostly read-only merchant verification diagnostic into an operational review workflow where platform staff can see why a store is blocked, what the admin should do next, what the merchant must fix, and which decisions are permitted.
- **Scope:** Admin merchant verification readiness checklist actions, table/search/metrics correctness, review decision workflow, KYC review API validation for requested changes, bank review action panel, focused regression tests, browser verification, and ops documentation.
- **Out of Scope:** DB migrations, production deploy/action, staging deploy, changing production config, adding new permission migrations, secrets, live payment-provider calls, live shipping-provider calls, and deleting the separate Platform Launch Gate engine.
- **Skills Used:** `acceptance-criteria-gate`, `design-ux-excellence-gate`, `premium-product-quality-council`, `priority-triage-gate`, `regression-safety-gate`, `implementation-quality-gate`, `test-strategy-gate`, `single-source-of-truth-gate`, `verification-before-completion`, `documentation-handoff-gate`, `evidence-led-reporting`, `environment-safety-gate`, `branch-pr-hygiene-gate`, `agent-permission-boundary`, plus `redesign-existing-projects`, `build-web-apps:react-best-practices`, and `browser:control-in-app-browser`.
- **Acceptance Criteria:**
  - [x] Publish-readiness checklist items include actionable admin destinations and merchant-facing instructions.
  - [x] Admin metrics respect the current filters/search instead of showing global counts that contradict visible rows.
  - [x] Empty search/filter results do not keep showing details for a hidden or stale selected store.
  - [x] Merchant verification approval is blocked until publish-readiness blockers are closed.
  - [x] Admins can choose clear verification decisions: approve, request changes, or reject.
  - [x] `needs_more_info` review decisions require and persist an admin reason, matching rejected decisions.
  - [x] Bank review has a dedicated action panel using existing review permissions and safe masked bank summaries.
  - [x] Full IBAN values are not displayed.
  - [x] PCI/Pentest/ASV/DR remains outside merchant verification.
  - [x] Browser verification confirms the local admin page shows the decision workflow, merchant instructions, disabled unsafe actions, and no stale-detail empty state.
- **Test Plan:** `pnpm vitest run tests/admin-merchant-verification.test.ts`; focused compliance/platform/RBAC/auth regression suite; `pnpm --filter @haa/admin-dashboard typecheck`; `pnpm --filter @haa/api typecheck`; `pnpm --filter @haa/admin-dashboard build`; in-app browser verification of `http://localhost:5175/compliance`; `pnpm check:skills`; `git diff --check`; final `pnpm preflight`.
- **Files Changed:** `apps/admin-dashboard/src/lib/merchantVerification.ts`, `apps/admin-dashboard/src/lib/api.ts`, `apps/admin-dashboard/src/pages/Compliance.tsx`, `apps/admin-dashboard/src/pages/KycReview.tsx`, `apps/api/src/routes/admin/index.ts`, `apps/api/src/routes/admin/tenants-stores.ts`, `tests/admin-merchant-verification.test.ts`, `docs/ops/TASK_TRACKER.md`, `docs/ops/CURRENT_STATE.md`, `docs/ops/ISSUE_KNOWLEDGE_BASE.md`, `docs/ops/REGRESSION_CHECKLIST.md`, `docs/ops/CHANGELOG_INTERNAL.md`, `docs/ops/SKILL_COMPLIANCE_REPORT_TASK_0136.md`.
- **Test Results:** `pnpm vitest run tests/admin-merchant-verification.test.ts` passed 1 file / 13 tests. Focused regression suite passed 6 files / 55 tests: `tests/admin-merchant-verification.test.ts`, `tests/admin-platform-compliance-gates.test.ts`, `tests/admin-permission-reflection.test.ts`, `tests/merchant-compliance-contract.test.ts`, `tests/g1-g10-engineering-prep.test.ts`, and `tests/admin-accountant-login.test.ts`. `pnpm --filter @haa/admin-dashboard typecheck` passed. `pnpm --filter @haa/api typecheck` passed. `pnpm --filter @haa/admin-dashboard build` passed. `pnpm check:skills` passed 43/43. Final `git diff --check` was clean. Final `pnpm preflight` passed. Browser verification confirmed `http://localhost:5175/compliance` renders "توثيق المتاجر", actionable merchant instructions, the review decision workflow, safe disabled actions for incomplete local sample stores, and no full IBAN text.
- **Root Cause:** TASK-0134 separated merchant verification from platform compliance correctly, but the first usable page was still closer to a review dashboard than a complete admin decision station. It explained blockers but did not fully guide the admin/merchant next action path, did not support "request changes", and could show stale details when filters returned no rows.
- **Verdict:** Decision workflow hardening is implemented and locally verified. Not pushed, not opened as PR, not merged, and not deployed.
- **Related Issues:** ISSUE-0072.

---

### TASK-0135: Local admin login schema-readiness fallback

- **Type:** Backend API / Testing / Documentation
- **Priority:** P1 High
- **Status:** Local scope verified; not pushed/deployed
- **Created:** 2026-06-30
- **Updated:** 2026-06-30
- **Branch:** `codex/merchant-compliance-readiness-fix`
- **PR:** Not opened
- **Original Request:** User opened the local admin dashboard at `http://localhost:5175/login`, saw "استجابة غير متوقعة من الخادم", then asked Codex to run/fix it.
- **Expanded Requirement:** Make the local admin login usable without running migrations or mutating the database, so the user can review the redesigned `توثيق المتاجر` page in the admin dashboard.
- **Scope:** Admin login service schema-readiness fallback, focused regression test, auth-core build for local dev server consumption, local API/browser verification, and ops documentation.
- **Out of Scope:** DB migration execution, altering local schema manually, production/staging deploy, broad admin-users route compatibility, secrets, live payment/shipping/provider calls, and changing admin RBAC semantics after `admin_role` exists.
- **Skills Used:** `acceptance-criteria-gate`, `regression-safety-gate`, `test-strategy-gate`, `implementation-quality-gate`, `verification-before-completion`, `documentation-handoff-gate`, `evidence-led-reporting`, `environment-safety-gate`, and `branch-pr-hygiene-gate`.
- **Acceptance Criteria:**
  - [x] `/admin/login` no longer fails because `users.admin_role` is absent in an unapplied local development schema.
  - [x] Base admin credential lookup does not select optional migration columns before password verification.
  - [x] Role lookup still uses `admin_role` when available, preserving role-scoped permissions for migrated environments.
  - [x] Missing `admin_role` column is treated as local schema readiness and falls back to default admin permissions instead of a 500.
  - [x] Regression test covers missing `admin_role` during login.
  - [x] Auth-core dist is rebuilt so the API dev server consumes the change.
  - [x] Local browser login reaches the admin app and opens `/compliance`.
- **Test Plan:** `pnpm vitest run tests/admin-accountant-login.test.ts`; `pnpm --filter @haa/auth-core typecheck`; `pnpm --filter @haa/api typecheck`; `pnpm --filter @haa/auth-core build`; local POST `/admin/login` through `http://localhost:5175`; in-app browser login and `/compliance` load.
- **Files Changed:** `packages/auth-core/src/admin-auth-service.ts`, `tests/admin-accountant-login.test.ts`, `docs/ops/TASK_TRACKER.md`, `docs/ops/CURRENT_STATE.md`, `docs/ops/ISSUE_KNOWLEDGE_BASE.md`, `docs/ops/REGRESSION_CHECKLIST.md`, `docs/ops/CHANGELOG_INTERNAL.md`, `docs/ops/SKILL_COMPLIANCE_REPORT_TASK_0135.md`.
- **Test Results:** Focused regression passed: `pnpm vitest run tests/admin-accountant-login.test.ts` => 1 file / 5 tests. `pnpm --filter @haa/auth-core typecheck` passed. `pnpm --filter @haa/api typecheck` passed. `pnpm --filter @haa/auth-core build` passed. Local login verification through `http://localhost:5175/admin/login` returned 200 with token redacted. In-app browser login succeeded and opened `http://localhost:5175/compliance` with heading "توثيق المتاجر".
- **Root Cause:** `AdminAuthService.login()` still selected `users.admin_role` in the initial base user query. On a local database where the admin-role migration has not been applied, the query failed before the service could verify credentials or fall back gracefully.
- **Verdict:** Login blocker is fixed locally and the admin verification page is open for review. Not pushed, not opened as PR, not merged, and not deployed.
- **Related Issues:** ISSUE-0071.

---

### TASK-0134: Merchant compliance readiness and admin merchant verification

- **Type:** Bug Fix / UX-UI Polish / Testing / Documentation
- **Priority:** P1 High
- **Status:** Local scope verified; not pushed/deployed
- **Created:** 2026-06-30
- **Updated:** 2026-06-30
- **Branch:** `codex/merchant-compliance-readiness-fix`
- **PR:** Not opened
- **Original Request:** User shared the staging admin compliance screen and prior-agent logs, requested a separate branch because other agents are working, then clarified that admin `/compliance` must be "توثيق المتاجر" / Merchant Verification, not owner/platform operational gates.
- **Expanded Requirement:** Keep merchant KYC/bank readiness safe, redesign admin `/compliance` as a merchant-store verification review surface, and explicitly separate Merchant Verification from Platform Launch Gates. PCI-style operational checks, penetration testing, ASV scans, and disaster-recovery planning must not appear as merchant requirements.
- **Scope:** Merchant KYC profile field mapping, merchant bank readiness/display contract, admin merchant verification normalization, publish-readiness pure function, admin verification table/detail panel, permission-gated review actions, focused regression tests, and ops documentation.
- **Out of Scope:** WhatsApp campaigns / TASK-0133, Account Security, DB schema changes, migrations, production deploy/action, staging publication, secrets, live payment/shipping/provider calls, full-IBAN display, and deletion of the separated Platform Launch Gate engine.
- **Skills Used:** `acceptance-criteria-gate`, `design-ux-excellence-gate`, `regression-safety-gate`, `implementation-quality-gate`, `test-strategy-gate`, `single-source-of-truth-gate`, `verification-before-completion`, `documentation-handoff-gate`, `evidence-led-reporting`, `environment-safety-gate`, `branch-pr-hygiene-gate`, `agent-permission-boundary`, `redesign-existing-projects`, and `build-web-apps:react-best-practices`.
- **Acceptance Criteria:**
  - [x] Merchant dashboard saves KYC fields using canonical API keys: `commercialRegistrationNumber`, `nationalIdOrIqama`, and `freelanceDocumentNumber`.
  - [x] API keeps backward compatibility for old merchant-dashboard aliases: `crNumber`, `nationalId`, and `freelanceDocNumber`.
  - [x] API profile responses do not return the full national ID; they return safe presence metadata and last four digits.
  - [x] Merchant bank readiness uses safe bank summaries (`ibanLast4` + `status`) and never requires or displays a full stored IBAN.
  - [x] Admin `/compliance` is renamed in the sidebar to "توثيق المتاجر" and shows merchant/store verification, not Platform Launch Gates.
  - [x] Admin list rows show store name/link, owner, entity type, verification, phone, registry/document, bank, payment, shipping, payout, publish readiness, risk, last update, and review action.
  - [x] Admin detail panel shows store data, owner data, phone/email verification, registry/freelance document, bank summary, uploaded documents, payment/shipping readiness, policies, publish checklist, review history, and internal notes.
  - [x] Bank display uses only `bankName`, `accountHolderName`, `ibanLast4`, `verificationStatus`, `rejectionReason`, `reviewedAt`, and `reviewedBy`.
  - [x] Publish readiness is calculated by a pure function returning `allowed`, `blockingReasons`, `warnings`, and `checklist`.
  - [x] Review approval/rejection actions are gated by existing `kyc.review`; bank review visibility uses existing bank/KYC permissions. No new permission migration was added.
  - [x] Platform Launch Gate logic remains separated in `platformComplianceGates.ts` and is not imported by the admin merchant verification page.
  - [x] Focused tests cover publish readiness, bank status/masking, registry/freelance document state, phone/email verification, rejected-bank blocking, unverified-phone blocking, full-IBAN non-exposure, and no operational-gate vocabulary in the merchant verification page.
  - [x] Final full verification (`admin build`, `check:skills`, `diff --check`, `preflight`) recorded before closure.
- **Test Plan:** `pwd`; startup `pnpm preflight`; read system map/current state/tracker/KB/decisions and skills; `pnpm ops:monitor`; focused merchant/admin compliance tests; admin typecheck/build; `pnpm check:skills`; `git diff --check`; final `pnpm preflight`; `git status --short --branch`.
- **Files Changed:** `apps/api/src/routes/compliance.ts`, `apps/merchant-dashboard/src/pages/Compliance.tsx`, `apps/merchant-dashboard/src/lib/complianceModel.ts`, `apps/admin-dashboard/src/pages/Compliance.tsx`, `apps/admin-dashboard/src/lib/merchantVerification.ts`, `apps/admin-dashboard/src/lib/platformComplianceGates.ts`, `apps/admin-dashboard/src/App.tsx`, `tests/merchant-compliance-contract.test.ts`, `tests/admin-platform-compliance-gates.test.ts`, `tests/admin-merchant-verification.test.ts`, `tests/admin-permission-reflection.test.ts`, `tests/g1-g10-engineering-prep.test.ts`, `docs/agent-os/ACTIVE_WORK.md`, `docs/ops/TASK_TRACKER.md`, `docs/ops/CURRENT_STATE.md`, `docs/ops/ISSUE_KNOWLEDGE_BASE.md`, `docs/ops/REGRESSION_CHECKLIST.md`, `docs/ops/CHANGELOG_INTERNAL.md`, `docs/ops/SKILL_COMPLIANCE_REPORT_TASK_0134.md`.
- **Test Results:** Startup `pnpm preflight` passed. `pnpm ops:monitor` exited 0 with no recommended tasks/incidents and only expected local dev-server warnings. Focused verification passed: `pnpm vitest run tests/admin-merchant-verification.test.ts tests/admin-platform-compliance-gates.test.ts tests/admin-permission-reflection.test.ts tests/merchant-compliance-contract.test.ts tests/g1-g10-engineering-prep.test.ts` => 5 files / 47 tests. `pnpm --filter @haa/admin-dashboard typecheck` passed. `pnpm --filter @haa/admin-dashboard build` passed. `pnpm check:skills` passed 43/43. `git diff --check` was clean. Final `pnpm preflight` passed, including workspace TypeScript typecheck.
- **Root Cause:** The first admin compliance interpretation mixed two different concepts: owner/platform launch gates and merchant-store verification. The merchant journey needs phone/email, legal document, bank, payment, shipping, policies, risk, and publish-readiness checks; operational platform controls must stay separate so a store is not blocked by platform-only duties.
- **Verdict:** Merchant verification redesign is implemented and locally verified. Not pushed, not opened as PR, not merged, and not deployed.
- **Related Issues:** ISSUE-0070.
...
- **Priority:** P1 High
- **Status:** Done; merged in PR #341 and published to staging
- **Created:** 2026-06-30
- **Updated:** 2026-06-30
