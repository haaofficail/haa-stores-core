...
| `apps/*/src/index.css`                                       | `single-source-of-truth-gate`, `build-on-existing-system-gate`, `regression-safety-gate`       | Global CSS vars — missing `:root` block breaks every Tailwind custom class in the app; verify `grep -c ":root"` before adding classes that depend on vars |
| `apps/*/tailwind.config.*`                                   | `single-source-of-truth-gate`, `build-on-existing-system-gate`                                 | Config references CSS vars that must already exist in `index.css`; cross-app copy requires grep verification in target app                                |
