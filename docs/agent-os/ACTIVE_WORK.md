...
**TASK-0131 — Admin Store Payment Settings save contract is complete and published to staging (2026-06-30).**

Branch: `codex/admin-store-payment-typed-cache`. PR: #341. Merge commit:
`719b234d9853182f45d7d65d012c1ae3e2fcd68d`.

The current focused admin gap is the Store Payment Settings save contract. The
dashboard posted `enabled`, `status: 'suspended' | 'not_configured'`, and
`supportedPaymentMethod: 'card'`, but the API validator expected legacy
`isEnabled`, rejected those page status values, and stripped
...
build, `pnpm check:skills` 43/43, clean `git diff --check`, `pnpm ops:monitor`
0 failures/no recommended tasks or incidents, `pnpm preflight`, and pre-commit
full `pnpm -r typecheck`. Remote verification passed: post-merge `main CI` run
`28413299676` succeeded, including E2E; `main Deploy` run `28413299656`
initially failed only in staging SSH warmup before deploy, then rerun staging
job `84327350186` succeeded and passed the official staging smoke gate. Public
smoke returned 200 for admin staging, staging `/health`, and the staging root.
Do not stage `storage/monitoring-events.ndjson`; it was generated by required
local monitoring and is outside this scope.
