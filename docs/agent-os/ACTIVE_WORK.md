...
**TASK-0139 — Admin tenant operating dossier is locally verified (2026-06-30).**

Branch: `codex/task-0139-tenant-dossier`. PR: not opened.
...
operating file: it aggregates stores, Merchant Verification readiness, blockers,
warnings, masked bank state, payment/payout/publish/risk state, visible payment
totals, latest payout/extract rows, settlement-batch rows, audit history, and
safe next-action links. The finance context is intentionally read-only; no
payout approval, transfer verification, settlement mutation, or full-IBAN reveal
was added.

Verification: startup `pnpm preflight`, startup `pnpm ops:monitor`, focused
source suite 3 files / 22 tests, `pnpm --filter @haa/admin-dashboard
typecheck`, `pnpm --filter @haa/admin-dashboard build`, `pnpm check:skills`
43/43, clean `git diff --check`, final `pnpm preflight`, and browser checks on
`http://localhost:5175/tenants` plus `http://localhost:5175/tenants/1` in
desktop and mobile viewports. Browser checks showed the decision center, store
journeys, finance context, audit context, no failed query, no full IBAN, and no
