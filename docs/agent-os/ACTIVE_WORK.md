...
**TASK-0130 — Admin beta direct-delete hardening is active (2026-06-30).**

The current branch is `codex/admin-improvement-round` based on `origin/main`.
The focused scope is the admin tenants/stores dangerous delete surface. Direct
tenant and store delete actions are removed from the admin UI, and direct store
DELETE now returns `FORBIDDEN_BETA_POLICY` before any hard-delete path, matching
DECISION-OS-014's beta rule: no hard delete anywhere. Tenant/store status
changes remain available through reason-gated `AdminDialog` flows and audited
API payloads.

Local verification currently passes: `pnpm ops:monitor`, focused deletion
...
build, `pnpm check:skills`, `git diff --check`, and `pnpm preflight`. PR checks
and staging publication are still pending until TASK-0130 is closed.
...
`codex/admin-improvement-round` contains only TASK-0130 admin delete hardening
plus required docs/test updates. Do not stage `storage/monitoring-events.ndjson`;
it was generated by required local monitoring and is not part of the PR.
