...
**TASK-0141 — Main change-password tenant context typecheck blocker is locally fixed (2026-07-01).**

Branch: `fix/main-change-password-tenant-id`. PR: not opened.

This is a small independent main-CI blocker fix created from `origin/main`,
separate from the TASK-0134..0140 stacked admin work. The blocker was:
`apps/api/src/routes/auth.ts(452,9): 'tenantId' does not exist in type
'ChangePasswordInput'`. The route now builds a local server-side
`changePasswordInput` object before calling `AuthFlowService.changePassword()`,
which preserves JWT-derived `tenantId` and `activeStoreId` as audit context
without expanding the `/auth/change-password` request schema. The account
security test now also guards that `changePasswordSchema` does not accept
tenant/store identifiers from the client.

Verification passed locally: `pnpm ops:monitor` exited 0 with no recommended
tasks/incidents, `pnpm --filter @haa/api typecheck`, focused auth/account
security suite 4 files / 34 tests, `pnpm typecheck`, `pnpm preflight`,
`pnpm --filter @haa/api build`, `pnpm check:skills` 43/43, and clean
`git diff --check`. Runtime `storage/monitoring-events.ndjson` generated by
monitoring was restored and kept out of scope.

Do not push without owner approval. After this PR is reviewed and merged, the
TASK-0134..0140 PR stack can be refreshed from the new `origin/main`: Base PR
to `main`, then TASK-0139 and TASK-0140 stacked on the Base branch.
